...
| `holding_back_solar` | Holding back solar · *Battery full, holding back solar* | `curtail` (`pv_spill_kw` > 0: battery full or charging at its maximum) — or `no_charge` spilling (room kept for a later fill; sub-label says when) · the `curtail` attribute is true when feed-in is negative, false when the cut is your export limit | the plan spills solar: at negative feed-in because exporting would cost you, or because the export limit is reached |
