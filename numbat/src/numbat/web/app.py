...
dashboard (numbat/frontend, built by Vite into web/dist). All URLs must stay
...
from numbat.config import EnvSettings, Settings
from numbat.config_store import SCHEMA_VERSION, ConfigController
from numbat.forecast.load import default_timezone
...
def _resolve_tz() -> ZoneInfo:
    ...
    except Exception:  # noqa: BLE001 - a bad NUMBAT_TZ shouldn't 500 the tool
        return ZoneInfo("UTC")
...
# Vite build output (numbat/frontend -> `bun run build`); gitignored, built by CI
...
class HealthState:
    ...
    def restart_grace(self) -> None:
        ...
        503 — and restart the add-on — the moment the user enables Numbat."""
...
def create_app(
    ...
    app = FastAPI(title="Numbat", version=__version__)
    health = state.health
    ...
    if controller is not None:
        ...
        async def export_config() -> JSONResponse:
            ...
            of the on-disk numbat-config.json, so an exported file, a filesystem
            backup, and the .bak are all interchangeable for import."""
            current = controller.current
            if current is None:
                return JSONResponse({"error": "configure Numbat first"}, status_code=409)
            stamp = datetime.now(_resolve_tz()).strftime("%Y-%m-%d")
            return JSONResponse(
                {"schema_version": SCHEMA_VERSION, "config": current.model_dump(mode="json")},
                headers={
                    "Content-Disposition": f'attachment; filename="numbat-config-{stamp}.json"'
                },
            )
    ...
    if (dist_dir / "index.html").exists():
        ...
        async def index() -> JSONResponse:
            return JSONResponse(
                {"error": "dashboard not built — run `bun run build` in numbat/frontend"},
                status_code=503,
            )
