...
def test_dashboard_served_from_dist(tmp_path):
    ...
    (tmp_path / "index.html").write_text(
        '<!doctype html><title>Numbat</title><script src="./assets/index-abc.js"></script>'
    )
    (tmp_path / "assets" / "index-abc.js").write_text("console.log('numbat')")
    client = TestClient(create_app(AppState(), dist_dir=tmp_path))
    index = client.get("/")
    assert index.status_code == 200
    assert "Numbat" in index.text
...
def test_health_grace_rearms_when_planning_resumes():
    ...
    # the watchdog would see 503 (stale last_success) the moment Numbat starts
...
def test_put_config_persists_and_wakes_the_loop(tmp_path):
    ...
    on_disk = json.loads((tmp_path / "numbat-config.json").read_text())
...
def test_export_config_downloads_the_store_document(tmp_path):
    ...
    assert disposition.startswith('attachment; filename="numbat-config-')
...
class FakeStatesClient:
    async def list_states(self):
        from numbat.ha.client import State
